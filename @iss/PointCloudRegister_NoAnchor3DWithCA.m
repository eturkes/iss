function o = PointCloudRegister_NoAnchor3DWithCA(o, y, x0, nTiles, Options)     %MADE A THE SAME FOR ALL TILES
...
% This does take chromatic aberration into consideration, so different transform 
% applied to all colour channels in a round even though the starting point 
% is the same.
%
% inputs:
...
for i=1:o.PcIter
    ...
    for t=1:nTiles
        ...
        for r=o.UseRounds 
            for b=o.UseChannels
                xTransformedAnchor(t,b,r) = {(x{t}*A(:,:,t,r,b))'};   
            end
        end
    end
    ...
    for r=o.UseRounds 
        for t=1:nTiles
            ...
            for b=o.UseChannels         
                ...
                A(:,:,t,r,b) = x{t}(UseMe{t,b,r}>0,:)\y{t,b,r}(MyNeighb{t,b,r},:);
            end
        end
    end
